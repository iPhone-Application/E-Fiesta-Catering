...
- (void)viewDidLoad {
    ...
    [ImgView sizeToFit];
    ImgView.image=[UIImage imageNamed:@"BirthdayBackground.png"];
    [self.tableView setBackgroundView:ImgView];
    [self setTitle:@"Birthday Party"];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==0)
    {
        BirthdayPredefined *objBirthdayPredefined=[self.storyboard instantiateViewControllerWithIdentifier:@"BirthdayPredefined"];
        [self.navigationController pushViewController:objBirthdayPredefined animated:YES];
        
        
    }
    else if(buttonIndex==1)
    {
        CustomizedPackage *objCustomizedPackage=[self.storyboard instantiateViewControllerWithIdentifier:@"BirthdayPredefinedGold"];
