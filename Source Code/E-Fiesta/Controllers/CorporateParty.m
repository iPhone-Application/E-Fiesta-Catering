...
@interface CorporateParty ()

@end

@implementation CorporateParty
...
- (void)viewDidLoad {
    ...
    [ImgView sizeToFit];
    ImgView.image=[UIImage imageNamed:@"CorporateBackground.png"];
    [self.tableView setBackgroundView:ImgView];
    [self setTitle:@"Corporate Party"];
}
...
{
    UIActionSheet *actSht=[[UIActionSheet alloc] initWithTitle:@"Please Select" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"10-50",@"50-100",@"100-200",@"200-500",@"500-1000",@"1000-1500",@"1500-2000",@">2000",nil];
    [actSht showInView:self.view];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==0)
    {
        CorporatePredefined *objCorporatePredefined=[self.storyboard instantiateViewControllerWithIdentifier:@"CorporatePredefined"];
        [self.navigationController pushViewController:objCorporatePredefined animated:YES];
        
        
    }
    else if(buttonIndex==1)
    {
        CustomizedPackage *objCustomizedPackage=[self.storyboard instantiateViewControllerWithIdentifier:@"CorporatePredefinedGold"];
