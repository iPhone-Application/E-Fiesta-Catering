...
- (void)viewDidLoad {
    ...
    [ImgView sizeToFit];
    ImgView.image=[UIImage imageNamed:@"GettogetherBackground.png"];
    [self.tableView setBackgroundView:ImgView];
    [self setTitle:@"Get-Together"];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==0)
    {
        GetTogetherPredefined *objGetTogetherPredefined=[self.storyboard instantiateViewControllerWithIdentifier:@"GetTogetherPredefined"];
        [self.navigationController pushViewController:objGetTogetherPredefined animated:YES];
        
        
    }
    else if(buttonIndex==1)
    {
        CustomizedPackage *objCustomizedPackage=[self.storyboard instantiateViewControllerWithIdentifier:@"GetTogetherPredefinedGold"];
